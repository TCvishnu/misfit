...
function caseNumber(roomCode: string) {
  const digits = roomCode.replace(/\D/g, '')
...
export function Game({ session, onExit }: { session: Session; onExit: () => void }) {
  ...
  if (!view) {
    return (
      <div className="grid min-h-dvh place-items-center">
        <span className="typed caret text-sm uppercase tracking-[0.2em] text-paper-3/60">
          Retrieving file
        </span>
      </div>
    )
  ...
  return (
    <div className="mx-auto flex min-h-dvh max-w-2xl flex-col gap-5 px-5 pb-28 pt-8">
      <Sheet tilt={-0.4}>
        <div className="flex items-start justify-between gap-4">
          <div>
            <div className={styles.field}>Case file</div>
            <div className="typed text-2xl leading-tight text-ink">
        ...
        <div className="flex items-end justify-between gap-4">
          <div>
            ...
            <div className="typed text-sm uppercase tracking-[0.14em] text-ink">
              {view.status === 'in_play'
                ? round
          ...
          <div className="text-right">
            ...
            <div className="typed text-sm text-ink">
      ...
      <div className="flex flex-wrap items-center gap-x-4 gap-y-2">
        <span className={styles.fieldDark}>Present</span>
        {view.players.map((p) => (
          <span key={p.player_code} className="flex items-center gap-2">
            ...
            <span className="text-sm text-paper-3">
              {p.name}
              {p.player_code === me?.player_code && (
                <span className="text-paper-3/50"> (you)</span>
              )}
            </span>
            <span className="typed text-xs text-paper-3/60">{p.score}</span>
          </span>
        ))}
      </div>
      ...
      {view.status === 'waiting' && (
        <Sheet tilt={0.4} className="text-center">
          ...
          <p className="typed mt-2 text-lg text-ink">Awaiting personnel</p>
        ...
        <>
          ...
          {round.phase === 'answering' && (
            <Statement
              view={view}
              round={round}
              onSubmit={(text) => act(() => api.answer(view.room_code, session.playerCode, text))}
            />
          )}
              ...
              <div className="stagger flex flex-col">
                {round.answers.map((a) => {
                  ...
                  return (
                    <div
                      key={a.player_code}
                      className={`flex items-center gap-3 border-b border-dashed border-ink-faint/40 py-2.5 last:border-0 ${
                        flagged ? 'bg-stamp/[0.07]' : ''
                      }`}
                    >
                      <Badge name={a.name} code={a.player_code} size="sm" flagged={flagged} />
                        ...
                        <button
                          className={
                            round.your_vote === a.player_code
                              ? 'typed border-2 border-stamp px-3 py-1.5 text-xs uppercase tracking-[0.12em] text-stamp'
                              : styles.key
                          }
                      ...
                      {round.phase === 'results' && votes > 0 && (
                        <span className="typed shrink-0 text-xs uppercase tracking-[0.1em] text-stamp">
          ...
          {round.phase === 'voting' && (
            <p className="text-center text-sm text-paper-3/60">
      ...
      {error && (
        <p className="border-l-4 border-stamp bg-stamp/10 px-3 py-2 text-sm text-paper">
          ⚠ {error}
        </p>
      )}

      <footer className="fixed inset-x-0 bottom-0 border-t border-paper-3/15 bg-desk/95 backdrop-blur">
        <div className="mx-auto flex max-w-2xl items-center gap-3 px-5 py-3">
          ...
          <button className="ml-auto text-xs text-paper-3/40 underline" onClick={onExit}>
            withdraw
          </button>
        </div>
      </footer>
...
function RoomCode({ code }: { code: string }) {
  ...
  return (
    <button
      className="typed text-sm text-pen underline decoration-dotted underline-offset-4"
      onClick={() => {
  ...
  return (
    <div className="flex flex-col gap-4">
      <Sheet tilt={-0.5} key={round.question ?? round.your_question}>
        ...
        <blockquote className="typed mt-3 text-balance text-xl leading-relaxed text-ink">
      ...
      {youAreTheMisfit && (
        <Sheet tilt={0.8} className="border-2 border-stamp/40">
          ...
          <div className="rule my-3" />
          <p className="typed text-lg text-ink">You were handed a different document.</p>
          <p className="mt-2 text-ink-soft">
            Yours read: “{round.your_question}”
          </p>
  ...
  return (
    <Sheet tilt={0.3}>
      ...
      <p className="mt-3 text-xs text-ink-faint">
        Once signed it cannot be withdrawn.
      </p>
    </Sheet>
  )
}
...
function Transcript({
  ...
  return (
    <Sheet tilt={-0.3}>
      <div className={styles.field}>Transcript of cross-examination</div>
      <div className="rule my-3" />

      <div className="flex max-h-56 flex-col gap-1.5 overflow-y-auto pr-1">
        {messages.length === 0 && (
          <p className="py-3 text-sm italic text-ink-faint">
            The room is silent. Somebody has to start.
          </p>
        )}
        {messages.map((m, i) => (
          <div key={i} className="flex gap-2 text-sm">
            <span
              className={`typed w-24 shrink-0 truncate uppercase tracking-[0.08em] ${
                m.player_code === me ? 'text-stamp' : 'text-ink-faint'
              }`}
            >
              {m.name}:
            </span>
    ...
    <Sheet tilt={-0.7}>
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          <p className="typed mt-2 text-2xl text-ink">
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          ...
          <p className="typed text-xl text-ink">
            {misfit?.name}
          ...
          <div className="mt-2 flex flex-col gap-1">
            {deltas.map(([code, points]) => (
              <div key={code} className="flex items-baseline gap-3 text-sm">
                <span className="flex-1 text-ink">{playerOf(code)?.name}</span>
    ...
    <Sheet tilt={0.4} className="torn">
      ...
      {ranked.map((p, i) => (
        ...
        >
          ...
          <span className={`flex-1 ${i === 0 ? 'typed text-ink' : 'text-ink-soft'}`}>
            {p.name}
